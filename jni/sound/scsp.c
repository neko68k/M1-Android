...
void *scsp_start(const void *config)
{
	...
	memset(&SCSPs[0], 0, sizeof(SCSPs[0]));
	memset(&SCSPs[1], 0, sizeof(SCSPs[1]));

	intf = config;

	// init the emulation
	SCSP_Init(&SCSPs[0], intf);
	SCSP_Init(&SCSPs[1], intf);

	// set up the IRQ callbacks
	{
		SCSPs[0].Int68kCB = intf->irq_callback[0];
	}

	SCSPs[0].SCSPRAM = (unsigned char *)intf->region[0];
	SCSPs[1].SCSPRAM = (unsigned char *)intf->region[1];
...
{
	SCSPs[which].SCSPRAM = base;
	SCSPs[which].DSP.SCSPRAM = base;
}


READ16_HANDLER( SCSP_0_r )
{
	struct _SCSP *SCSP = &SCSPs[0];
