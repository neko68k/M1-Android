...
void m1setfile(char *mstart, int mend)
{
	fstart = mstart;
	offset = 0;
	eof = 0;
...
void MPEG_Set_Loop(char *loop, int loopend)
{
	lstart = loop;
	lend = loopend;
}
...
int m1fread(unsigned char *buf, int size1, int size2, void *f)
{
	...
	if (in_file) return fread(buf, size1, size2, f);
...
{
	if (in_file) return fseek(f, offs, whence);
