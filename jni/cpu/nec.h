...
#define GetMemB(Seg,Off) ((UINT8)nec_readmem((DefaultBase(Seg)+(Off))))
#define GetMemW(Seg,Off) ((UINT16) nec_readmem((DefaultBase(Seg)+(Off))) + (nec_readmem((DefaultBase(Seg)+((Off)+1)))<<8) )

#define PutMemB(Seg,Off,x) { nec_writemem((DefaultBase(Seg)+(Off)),(x)); }
...
#define ReadByte(ea) ((BYTE)nec_readmem((ea)))
#define ReadWord(ea) (nec_readmem((ea))+(nec_readmem(((ea)+1))<<8))
#define WriteByte(ea,val) { nec_writemem((ea),val); }
#define WriteWord(ea,val) { nec_writemem((ea),(BYTE)(val)); nec_writemem(((ea)+1),(val)>>8); }

#define read_port(port) nec_readport(port)
#define write_port(port,val) nec_writeport(port,val)

#define FETCH (nec_readmem((I.sregs[CS]<<4)+I.ip++))
#define FETCHOP (nec_readop((I.sregs[CS]<<4)+I.ip++))
#define FETCHWORD(var) { var=nec_readmem((((I.sregs[CS]<<4)+I.ip)))+(nec_readmem((((I.sregs[CS]<<4)+I.ip+1)))<<8); I.ip+=2; }
...
#define PEEK(addr) ((BYTE)nec_readmem(addr))
#define PEEKOP(addr) ((BYTE)nec_readop(addr))

#define GetModRM UINT32 ModRM=nec_readmem((I.sregs[CS]<<4)+I.ip++)
