...
{
	return (nec_fullCount - nec_ICount);
}

void nec_yield(void)
{
	nec_fullCount = nec_getcycles();
	...
	switch( regnum )
	{
		...
		default:
			if( regnum <= REG_SP_CONTENTS )
			{
				...
				return nec_readmem( offset ) | ( nec_readmem( offset + 1) << 8 );
			}
	}
	return 0;
}
...
void nec_set_reg(int regnum, unsigned val)
{
	switch( regnum )
	{
		...
		default:
			...
			{
				unsigned offset = (((I.sregs[SS]<<4) + I.regs.w[SP])) + 2 * (REG_SP_CONTENTS - regnum);
				nec_writemem( offset, val & 0xff );
				nec_writemem( offset+1, (val >> 8) & 0xff );
			}
    }
}
...
#else
	sprintf( buffer, "$%02X", nec_readmem(pc) );
	return 1;
#endif
}
