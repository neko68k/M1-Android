...
//#define h8_mem_read8(x)      h8_mem_read8(x)
//#define h8_mem_read16(x)     h8_mem_read16(x)
//#define h8_mem_write8(x, y)  h8_mem_write8(x, y)
//#define h8_mem_write16(x, y) h8_mem_write16(x, y)
extern UINT8 h8_mem_read8(unsigned int address);
extern UINT16 h8_mem_read16(unsigned int address);
extern void h8_mem_write8(unsigned int address, UINT8 data);
extern void h8_mem_write16(unsigned int address, UINT16 data);
...
{
	data32_t result = h8_mem_read16(address) << 16;
	return result | h8_mem_read16(address + 2);
}

INLINE void h8_mem_write32(offs_t address, data32_t data)
{
	h8_mem_write16(address, data >> 16);
	h8_mem_write16(address + 2, data);
}
...
static int h8_fullcount;

int h8_getcycles(void)
{
	return (h8_fullcount - h8_cyccnt);
}

void h8_yield(void)
{
	h8_fullcount = h8_getcycles();
	h8_cyccnt = 0;
}

int h8_execute(int cycles)
{
	UINT16 opcode = 0;

	h8_cyccnt = cycles;
	h8_fullcount = h8_cyccnt;
	...
	if (h8.h8err) 
	{
		logerror("H8/3002: Unknown opcode (PC=%x)  %x", h8.ppc, opcode);
	}
...
static void h8_set_info(UINT32 state, union cpuinfo *info)
{
	...
	default:
		logerror("h8_set_info unknown request %x\n", state);
	}
}
...
//	case CPUINFO_PTR_INTERNAL_MEMORY_MAP + ADDRESS_SPACE_IO:      info->internal_map = 0; break;
	...
	default:
		logerror("h8_3002_get_info unknown request %x\n", state);
	}
}
