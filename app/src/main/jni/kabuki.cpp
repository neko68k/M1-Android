...
{
	unsigned char *rom = memory_region(RGN_CPU1);
	int diff = memory_region_length(RGN_CPU1) / 2;
...
{
	UINT8 *rom = memory_region(RGN_CPU1);
	int diff = memory_region_length(RGN_CPU1) / 2;
	int i;
//	unsigned char temp[0x8000];

	kabuki_decode(rom,rom+0x8000,rom,0x0000,0x8000,swap_key1,swap_key2,addr_key,xor_key);

	// The result needs to be swapped in order for stuff to work properly!
	//memcpy(temp,rom+0x8000,0x8000);
	//memcpy(rom+0x8000,rom,0x8000);
	//memcpy(rom,temp,0x8000);
	...
	{
		FILE *f;
		f = fopen("pang_data.bin","wb");
		//fwrite(rom,1,0x10000,f);
...
// For brd_mitchell.cpp
void mgakuen2_decode() { mitchell_decode(0x76543210,0x01234567,0xaa55,0xa5); }
void pang_decode()     { mitchell_decode(0x01234567,0x76543210,0x6548,0x24); }
void cworld_decode()   { mitchell_decode(0x04152637,0x40516273,0x5751,0x43); }
void hatena_decode()   { mitchell_decode(0x45670123,0x45670123,0x5751,0x43); }
void spang_decode()    { mitchell_decode(0x45670123,0x45670123,0x5852,0x43); }
void spangj_decode()   { mitchell_decode(0x45123670,0x67012345,0x55aa,0x5a); }
void sbbros_decode()   { mitchell_decode(0x45670123,0x45670123,0x2130,0x12); }
void marukin_decode()  { mitchell_decode(0x54321076,0x54321076,0x4854,0x4f); }
void qtono1_decode()   { mitchell_decode(0x12345670,0x12345670,0x1111,0x11); }
void qsangoku_decode() { mitchell_decode(0x23456701,0x23456701,0x1828,0x18); }
void block_decode()    { mitchell_decode(0x02461357,0x64207531,0x0002,0x01); }
