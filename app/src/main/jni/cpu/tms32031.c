...
#define ROPCODE(pc)	tms32031_read32((pc) << 2) 
#define OP		tms32031.op

#define RMEM(addr)	tms32031_read32((addr) << 2)
#define WMEM(addr,data)	tms32031_write32((addr) << 2, data)
#define UPDATEPC(addr)	
...
void tms32031_get_context(void *dst)
{
	/* copy the context */
	if (dst)
...
{
	tms32031.irq_callback = callback;
}
...
static int tms32031_fullCount;

int tms32031_getcycles(void)
{
	return (tms32031_fullCount - tms32031_icount);
}

void tms32031_yield(void)
{
	tms32031_fullCount = tms32031_getcycles();
