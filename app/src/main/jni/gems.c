...
unsigned int find_pattern(unsigned char *pat, int patlen)
{
	int ofs = 0;

	while (ofs < 8*1024*1024)
	{
		if (!memcmp(&ROM[ofs], pat, patlen))
		{
			return ofs;
		}
...
unsigned int find_pattern_ex(unsigned char *pat, int patlen, int ofs)
{
	if (ofs < 0) ofs = 0;
...
unsigned int find_patterns_ex(unsigned char *pat, int patlen, unsigned char *pat2, int patlen2, int ofs)
{
	while (ofs < 8*1024*1024)
	{
		if ((!memcmp(&ROM[ofs], pat, patlen)) || (!memcmp(&ROM[ofs], pat2, patlen2)))
		{
			return ofs;
		}
...
static int scanbacktorte(int start)
{
	...
	while ((ROM[temp] != 0x4e) || (ROM[temp+1] != 0x75))
	{
		temp -= 2;
	}
...
static int findstop(int start)
{
	...
	while (((ROM[temp] != 0x4e) || (ROM[temp+1] != 0xba) || (ROM[temp+4] != 0x70) || (ROM[temp+5] != 0x12)) && (temp < 8*1024*1024))
	{
		temp += 2;
	}
...
static int findstop2(int start)
{
	...
	while (((ROM[temp] != 0x4e) || (ROM[temp+1] != 0xb9) || (ROM[temp+6] != 0x70) || (ROM[temp+7] != 0x12)) && (temp < 8*1024*1024))
	{
		temp += 2;
	}
...
static int findstop3(int start)
{
	...
	while (((ROM[temp] != 0x61) || (ROM[temp+1] != 0x00) || (ROM[temp+4] != 0x70) || (ROM[temp+5] != 0x12)) && (temp < 8*1024*1024))
	{
		temp += 2;
	}
...
{
	if ((ROM[gemsinit+12] == 0x70) && (ROM[gemsinit+13] == 0xff) && (ROM[gemsinit+14] == 0x2f) && (ROM[gemsinit+15] == 0x00))
	{
		return 1;
	}
...
static int find_gemsstart(unsigned char *startpat, int startpatlen, unsigned char *pat, int patlen)
{
	...
	while (1)
	{
		printf("Candidate (%x): %02x %02x %02x %02x %02x %02x %02x %02x\n", gemsstart, ROM[gemsstart-6], ROM[gemsstart-5], ROM[gemsstart+4], ROM[gemsstart+5], ROM[gemsstart+6], ROM[gemsstart+7], ROM[gemsstart+8], ROM[gemsstart+9]);

		if (((ROM[gemsstart-6] == 0x4e) || (ROM[gemsstart-6] == 0x61)) && ((!memcmp(&ROM[gemsstart+8], pat, patlen)) || (!memcmp(&ROM[gemsstart+6], pat, patlen)) || (!memcmp(&ROM[gemsstart+4], pat, patlen))) )
		{
			return scanbacktorte(gemsstart-2);
		}
		else
		{
			if ((ROM[gemsstart-4] == 0x4e) && (ROM[gemsstart-3] == 0xba) && (ROM[gemsstart+2] == 0x4e) && (ROM[gemsstart+3] == 0xba))
			{
				return gemsstart-4;
			}
...
{
	FILE *f;
	int z80prg, gemsloadz80, gemsinit, type, initcode, gemsstart, gemsstop;
	int found;

	memset(ROM, 0, 8*1024*1024);

	f = fopen(argv[1], "rb");
	fread(ROM, 8*1024*1024, 1, f);
	fclose(f);
	...
	if (!initcode)
	{
		...
		while (!found)
		{
			...
			if (!initcode) 
			{ 
			 	if ((ROM[gemsinit+0x18] == 0x2f) && (ROM[gemsinit+0x19] == 0x3c) && (ROM[gemsinit+0x20] == 0x2f) && (ROM[gemsinit+0x21] == 0x3c))
				{
			...
			{
				offset = ROM[initcode+2]<<8 | ROM[initcode+3];
	...
	if ((ROM[initcode-6] == 0x2f) && (ROM[initcode-5] == 0x3c))
	{
		printf("Found init call at %08x with move.l parameters\n", initcode);
	}
	else if ((ROM[initcode-6] == 0x48) && (ROM[initcode-5] == 0x79)) 
	{
