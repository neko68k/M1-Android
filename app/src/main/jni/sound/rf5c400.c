...
static struct rf5c400_info *our_info;
...
static void rf5c400_update(int num, INT16 **outbuf, int length)
{
	int i, ch;
	struct rf5c400_info *info = our_info;
	INT16 *rom = info->rom;
	INT32 *buffer[2];
	INT32 bspace[44100/30], bspace1[44100/30];
	...
	memset(buffer[0], 0, length*sizeof(INT32));
	memset(buffer[1], 0, length*sizeof(INT32));
	...
	for (i=0; i < length; i++)
	{
		outbuf[0][i] = buffer[0][i]>>4;
		outbuf[1][i] = buffer[1][i]>>4;
	}
}
...
static void rf5c400_init_chip(struct rf5c400_info *info, int sndindex)
{
	...
	sprintf(buf[0], "RF5c400 L");
	sprintf(buf[1], "RF5c400 R");
	bufp[0] = buf[0];
	bufp[1] = buf[1];
	...
	info->stream = (void *)(size_t)stream_init_multi(2, bufp, vol, Machine->sample_rate, 0, rf5c400_update);
}
...
{
	const struct RF5C400interface *intf = msound->sound_interface;

	rf5c400_start(0, 44100, intf);
