...
public class Semaphore {
	...
	public synchronized void sem_wait(String currentThreadName) {
		...
		do {
			...
			try {
				if (doDebug)
					System.out.println(currentThreadName + " Waiting for "
	...
	public synchronized void sem_signal(String currentThreadName) {
		...
		/*
		 * * Check to see if a thread is waiting for a signal.* If not, then
		 * remember that a signal was generated without* a wait.
		 */
		if (numWait == 0) {
			s = s + 1;
			return;
		...
		do {
			...
			try {
				if (doDebug)
					System.out.println(currentThreadName + " Notify for "
							+ currentSemaphoreName);
